...
export function ScanResultCard({ scan }: ScanResultCardProps) {
  ...
  const isHighRisk = result.includes("melanoma") || result.includes("high") || result.includes("malignant");
  ...
  return (
    ...
    >
      <div className="relative h-64 bg-slate-100">
        ...
        {Array.isArray(scan.detections) && (scan.detections as any[]).map((det: any, i: number) => {
          const [x1, y1, x2, y2] = det.box;
          return (
            <div 
              key={i}
              className="absolute border-2 border-yellow-400 rounded-lg shadow-[0_0_15px_rgba(250,204,21,0.5)] animate-pulse"
              style={{
                left: `${x1 * 100}%`,
                top: `${y1 * 100}%`,
                width: `${(x2 - x1) * 100}%`,
            ...
            >
              <div className="absolute -top-6 left-0 bg-yellow-400 text-yellow-900 text-xs font-bold px-2 py-0.5 rounded whitespace-nowrap">
                Detected Lesion {Math.round(det.confidence * 100)}%
              </div>
            </div>
          );
        })}
        ...
        {isHighRisk ? (
          <div className="space-y-4">
            <div className="bg-red-50 border border-red-100 rounded-xl p-4 text-sm text-red-800 leading-relaxed">
              <strong>Attention needed:</strong> {scan.analysis || "The AI model has detected patterns consistent with melanoma. This is not a diagnosis, but immediate consultation with a dermatologist is strongly recommended."}
            </div>
          ...
          <div className="space-y-4">
            <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 text-sm text-emerald-800 leading-relaxed">
              <strong>Analysis findings:</strong> {scan.analysis || "The AI did not detect high-risk patterns. However, always monitor your skin for changes."}
            </div>
